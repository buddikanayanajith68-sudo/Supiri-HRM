        ...
        employees.forEach(emp => {
            ...
            tr.innerHTML = `
                <td class="py-4 px-6 border-b border-slate-100">
                    <span class="text-sm font-mono text-indigo-600 font-semibold bg-indigo-50 px-2 py-1 rounded-md">EMP-${String(emp.id).padStart(3, '0')}</span>
                </td>
                <td class="py-4 px-6 border-b border-slate-100">
                    <div class="flex items-center gap-3">
                        <img src="${getAvatar(emp.name)}" class="w-10 h-10 rounded-full shadow-sm">
        ...
        recent.forEach(emp => {
            ...
            tr.innerHTML = `
                <td class="py-3 px-4 border-b border-slate-100">
                    <span class="text-sm font-mono text-indigo-600 font-semibold">EMP-${String(emp.id).padStart(3, '0')}</span>
                </td>
                <td class="py-3 px-4 border-b border-slate-100">
                    <div class="flex items-center gap-3">
                        <img src="${getAvatar(emp.name)}" class="w-8 h-8 rounded-full">
...
document.getElementById('attendance-form')?.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if(conflict) {
            Swal.fire({ icon: 'warning', title: 'Already Logged!', text: 'Attendance for this employee has already been logged for this date.', showConfirmButton: true });
            return;
        }
...
async function editAttendance(id) {
    ...
    if(rec) {
        ...
        document.getElementById('att-form-title').innerText = 'Edit Attendance';
...
async function loadAttendance() {
    ...
    recent.forEach(rec => {
        ...
        tr.innerHTML = `
            ...
            <td class="py-3 px-6 text-sm font-mono text-indigo-600 font-semibold">${emp ? `EMP-${String(emp.id).padStart(3, '0')}` : '-'}</td>
            <td class="py-3 px-6 text-sm font-medium text-slate-800">${emp ? emp.name : 'Deleted'}</td>
            <td class="py-3 px-6 text-sm text-slate-500">${rec.inTime || '-'} to ${rec.outTime || '-'}</td>
...
async function generateReport() {
    ...
    let totalWorkingDays = records.length;
    let expectedManDays = standardDays;
    
    if(empIdVal === 'ALL') {
        const activeCount = employees.filter(e => e.status === 'Active').length;
        expectedManDays = standardDays * (activeCount || 1);
    }
    
    let totalLeaves = expectedManDays - totalWorkingDays;
    if(totalLeaves < 0) totalLeaves = 0;
    
    document.getElementById('report-total-hours').innerText = totalHours + ' hrs';
    ...
    if(records.length === 0) emptyState.classList.remove('hidden');
        ...
        records.forEach(rec => {
            ...
            tr.innerHTML = `
                ...
                <td class="py-2 px-6 text-sm font-mono text-indigo-600 font-semibold">${emp ? `EMP-${String(emp.id).padStart(3, '0')}` : '-'}</td>
